...
from codescripts.main_extraction import get_nutrition, get_recipe, get_suggestions
...
def suggestions_controller():
    ...
    if (content_type == 'application/json'):
            try:
                ...
                item = json['food_item']
                nutrient = json['nutrient']
                suggested_items = get_suggestions(nutrient,item)
                if suggested_items!=[]:
                    data = {}
                    data.update({'suggested_items':f'{suggested_items}'})
                ...
                data = {}
                message = "Exception encountered while fetching suggestions. Check JSON body"
                code = 401
                status = False
    else:
        data = {}
